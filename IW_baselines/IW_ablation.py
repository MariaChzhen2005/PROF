...
parser.add_argument('--exp_name', type=str, default='vanilla_update',
                    help='save name')
...
def update_parameters(agent, sigma=0.1, K = 4):
    loader = agent._get_training_samples()
    for i in range(K):
        for states, actions, disturbance, advantages, old_logprobs, x_uppers, x_lowers in loader:
            n_batch = states.shape[0]
            # pdb.set_trace()
            mu, _ = agent.policy.nn(state, disturbance, disturbance.transpose(0, 1))#,  x_lowers = x_lowers, x_uppers = x_uppers)# T x n x n_action
            #mus, sigma_sqs, proj_loss = self.policy.forward(states, ) # x, u: T x N x Dim.
            sigma_sqs = torch.ones_like(mus) * sigma**2
            
            log_probs, entropies = agent.evaluate_action(mus[0], actions, sigma_sqs)

            ratio = torch.exp(log_probs.squeeze()-old_logprobs)
            surr1 = ratio * advantages
            surr2 = torch.clamp(ratio, 1-agent.clip_param, 1+agent.clip_param) * advantages
            loss  = -torch.min(surr1, surr2).mean()
            agent.optimizer.zero_grad()
            ## Auxiliary losses
            loss -= torch.mean(entropies) * 0.01
            loss += agent.lam * proj_loss

            loss.backward()
            nn.utils.clip_grad_norm_(agent.policy.nn.parameters(), 100)
            self.optimizer.step()
...
def main():
    ...
    for i_episode in range(tol_eps):
        ...
        for t in range(n_step):
            ...
            mu, _ = agent.policy_old.nn(state, dt.unsqueeze(1))# T x n x n_action
            ...
            #pdb.set_trace()
            '''
            action, old_logprob = agent.select_action(mu[0], sigma_sq[0], u_limits = None)
            agent.memory.actions.append(action.detach().clone())
            agent.memory.old_logprobs.append(old_logprob.detach().clone())
            
            ## Project without Gradient
            mu[0] = action
            mu = mu.squeeze().detach()
            #pdb.set_trace()
            try:
                action_feasible = agent.policy_old.proj_layer(state[0], dt[:, :-1],
                                  mu, torch.zeros_like(mu), torch.zeros_like(mu),
                                  torch.tensor(x_upper).float(),
                                  torch.tensor(x_lower).float(),
                                  torch.tensor(agent.policy_old.u_upper.value).float(),
                                  torch.tensor(agent.policy_old.u_lower.value).float()
                                  )
                action = action_feasible[0][0]
            except:
                ## The feasible set is empty; Use some heuristics
                sp = np.mean((x_lower+x_upper)/2)
                if state.item() < sp:
            ...
            '''
            bl = obs_2017['rewards'][cur_time]
            rewards.append((reward-bl) / multiplier)
            '''
        ...
        if args.update_episode == -1:
            agent.memory.clear_memory() # Prevent memory overflow
